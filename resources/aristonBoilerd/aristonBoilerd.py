    ...
    if not JEEDOM_SOCKET_MESSAGE.empty():
        ...
        try:
            if message['action'] == 'getDatas':
                ...
                try:
                    value = message['value']
                    if isinstance(value, str) and value.startswith('OperationMode.'):
                        mode_str = value.split('.')[-1]
                    ...
                    logging.error(f"Invalid operation mode value: {message['value']} ({e})")
                    return
                if ariston_conn is not None:
                    try:
...
def init_ariston_connexion():
    global ariston_conn
    try:
        ariston_conn = AristonBoilerControl(_email, _password, poll_interval=5, quiet_login=True)
        ariston_conn.login()
